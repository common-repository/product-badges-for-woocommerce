...
function pbfw_shortcode_callbackfuncion(){
    ob_start();
    if (is_shop() && !is_admin()){
        echo "<div class='pbfw_square_data_main'>";
        echo PBFW_frontdesign();
        echo "</div>";
    }
...
function pbfw_product_badges_init() {
    ...
    add_shortcode('pbfw-product-badge', 'pbfw_shortcode_callbackfuncion');
}

add_filter( 'woocommerce_product_get_image', 'acoplwBadge' , 100000, 2 );

function acoplwBadge($productThumb, $product = false){
    
   if (is_shop() && !is_admin() && !is_single()){

    return $productThumb.PBFW_frontdesign();
